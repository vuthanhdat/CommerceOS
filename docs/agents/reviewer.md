...
Default model profile: **gpt-5.6-terra**, reasoning effort **medium**, service tier **standard** (Fast disabled unless the human explicitly overrides it). Route high-risk architecture/security/accounting/payment/concurrency ambiguity back to the appropriate planning role rather than silently changing the execution profile.
