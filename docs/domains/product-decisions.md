...
- **Decision:** MVP uses explicit entitlement types rather than one generic limit rule. Capability flags such as scheduled ingestion/API access are hard gates at the owning command boundary while preserving read/history/recovery access. Counted resource limits such as `MaxActiveMemberships` and `MaxWarehouses` are hard **growth/activation** limits: creation/activation that would exceed the limit is rejected, but existing over-limit resources caused by a pending downgrade are not destroyed and remain readable/manageable for remediation. `MaxActiveMemberships` counts every Active Owner/Admin/Staff/Viewer Membership under the approved `PD-044` catalog. Order-volume is soft-warning only under PD-051. Overage billing is not supported. `Unlimited` is an explicit entitlement value, never a missing record.
...
### PD-054 — First scheduled Product promotional-price policy

- **Status:** Resolved for first Pricing/Promotion slice
- **Question:** What exact promotion type, lifecycle, scheduling interval, authority, price/base-price interaction, public presentation, historical provenance, Accounting/refund treatment and plan availability define the first Pricing & Promotion capability?
- **Decision:** The first Pricing & Promotion slice is one **scheduled Product promotional price** for one Tenant-owned Product, applying to all public-storefront shoppers. The accepted Promotion stores an explicit final promotional unit price in VND whole đồng; percentage/fixed-reduction formulas are not supported. The first slice has no persisted merchant-editable Draft: Owner/Admin submit complete schedule terms in one create-and-schedule action. Accepted terms are immutable; a change requires cancellation, when still meaningful, followed by a new Promotion. `Upcoming`, `Active` and `Expired` are temporal interpretations, not activation/expiry commands. The effective interval is finite, start-inclusive/end-exclusive `[EffectiveFrom, EffectiveUntil)`, may start at the current authoritative instant or later, and cannot be backdated. Merchant-facing input/display uses the Tenant IANA timezone but must resolve to unambiguous instants; nonexistent/ambiguous local times are rejected rather than guessed. Cancellation of an Upcoming/Active Promotion is terminal and truncates actual eligibility at `CancelledAt`; previously accepted Orders are unchanged. For one Tenant + Product, accepted effective intervals may be adjacent but may not overlap; there is no stacking or priority engine. Scheduling requires the Product to be currently Published and requires promotional Money `>= 0` and strictly `<` the current Catalog base price; zero promotional price is allowed when base price is positive. Pricing never overrides Catalog sellability. During an otherwise-active Promotion, the effective unit price is the lower of the **current Catalog base price** and the immutable scheduled promotional unit price. Therefore a later Catalog base-price decrease to equal/below the promotional price makes the Promotion temporarily non-beneficial/not applied rather than raising shopper price or cancelling Catalog's change; if base price rises again before end/cancel, the Promotion may apply again. Unpublished Products are not sellable; if republished before the interval ends they may become eligible again, while Archived Products remain terminal under Catalog policy. Future scheduled Promotions are not publicly exposed before start. While a Promotion actually lowers the price, public presentation may expose base price, effective promotional price, a generic promotional indication and the end instant; no countdown, coupon, percentage-saved calculation, marketing-label engine or future-promotion preview is required. Checkout remains governed by PD-011: any difference between shopper-confirmed estimate and current authoritative effective price requires reconfirmation and no Order is created for that attempt. Sales must retain immutable accepted commercial evidence sufficient to stand alone without mutable Pricing reads, including accepted base unit price, effective unit price, applied PromotionId when applicable, accepted promotional unit price when applied, and authoritative pricing-evaluation instant. Because accepted first-slice Promotion terms are immutable, PromotionId is the stable terms identity and no merchant-editable version lifecycle is required. Scheduled/cancelled/expired Promotion records are retained non-destructively. Accounting and refunds use the effective amount actually accepted on the Sales Order; no contra-revenue/discount account is introduced for this slice and current Catalog/Pricing values never reprice refund/accounting history. Owner/Admin alone may schedule/cancel Promotions; Staff/Viewer cannot mutate Pricing. No new Pricing-specific subscription entitlement or plan-name gate is introduced: a Tenant otherwise eligible for ordinary Catalog/Storefront commerce may use the first slice.
- **Rationale:** A single immutable scheduled final price exercises a real Pricing bounded context, time-based authority, checkout repricing and historical provenance without prematurely creating a generic promotion engine. Create-and-schedule plus cancel/recreate avoids revision semantics and makes accepted commercial terms explainable. `[from, until)` intervals make adjacency deterministic. Using `min(current base, promotional price)` lets Catalog remain free to change its base price and guarantees a Promotion cannot increase shopper price; requiring the promotional price to be below base at scheduling catches nonsensical schedules early while preserving independent later Catalog changes. Net accepted-order accounting avoids introducing contra-revenue policy solely for one simple offer type. Excluding plan-specific gating avoids silently changing immutable PlanVersion/EntitlementSet commercial terms merely to ship a post-MVP learning slice.
- **Decision gate:** Resolved for TASK-0241. Material expansion beyond this first slice requires a new product decision before technical design/implementation.
- **Affected tasks:** TASK-0241, TASK-0242, TASK-0243; future Pricing/Promotion, Storefront/Checkout, Sales snapshot, Refund and reporting work.
- **Approved policy constraint:** Pricing may lower an eligible Product's current commercial price but never grant sellability, never trust client discounts/timestamps, never stack first-slice Promotions, and never rewrite historical Orders. Percentage/fixed discounts, coupons, manual order discounts, Category/cart/order-wide offers, customer segments, price lists, quantity tiers/Buy-X-Get-Y, variants, flash-sale orchestration, promotion-specific plan packaging and gross-discount/contra-revenue accounting remain out of scope.
- **Approved by:** CommerceOS human Product Owner — explicit TASK-0241 design instruction with delegated resolution of remaining first-slice details
- **Approved on:** 2026-08-14
- **Affected baseline documents updated:** `docs/domains/pricing-promotion.md`, `docs/domains/product-decision-reconciliation.md`, requirement/planning references in the TASK-0241 completion pass.
- **Affected candidate tasks notified:** TASK-0242 remains blocked only by its independent TASK-0234 architecture-audit dependency; TASK-0243 remains blocked by TASK-0242.
