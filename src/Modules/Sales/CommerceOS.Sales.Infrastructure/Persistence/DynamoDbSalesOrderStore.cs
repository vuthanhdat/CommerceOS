...
public sealed class DynamoDbSalesOrderStore(IAmazonDynamoDB client, DynamoDbSalesOptions options) : ISalesOrderStore, IRefundStore
{
    ...
    public async Task<IReadOnlyList<RefundRequest>> ListRefundsAsync(TrustedSalesContext context, CancellationToken ct)
    {
        var response = await client.QueryAsync(new() { TableName = options.TableName, KeyConditionExpression = "PK = :pk AND begins_with(SK, :prefix)", ExpressionAttributeValues = new() { [":pk"] = S(P(context.TenantId)), [":prefix"] = S("REFUND#") } }, ct);
        return response.Items.Select(ReadRefund).OrderByDescending(refund => refund.RequestedAt).ToArray();
    }
    ...
    private static Dictionary<string, AttributeValue> OrderItem(SalesOrder order) => new() { ["PK"] = S(P(order.TenantId)), ["SK"] = S($"ORDER#{E(order.Id.Value)}"), ["OrderId"] = S(order.Id.Value), ["TenantId"] = S(order.TenantId.Value), ["Lines"] = S(JsonSerializer.Serialize(order.Lines)), ["TotalVnd"] = N(order.TotalVnd), ["Guest"] = S(JsonSerializer.Serialize(order.Guest)), ["Status"] = S(order.Status.ToString()), ["Revision"] = N(order.Revision), ["PlacedAt"] = S(order.PlacedAt.ToString("O", CultureInfo.InvariantCulture)), ["ProcessId"] = S(order.Process.Id), ["ExecutionIdentity"] = S(order.Process.WorkflowExecutionIdentity), ["StartPending"] = new() { BOOL = order.Process.StartPending }, ["Sources"] = S(JsonSerializer.Serialize(order.AcceptedSources)) };
