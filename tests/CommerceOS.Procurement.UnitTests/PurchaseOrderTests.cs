...
public sealed class PurchaseOrderTests
{
    ...
    private sealed class Store(Supplier supplier, PurchaseOrder order) : IProcurementStore { public PurchaseOrder? Order { get; private set; } = order; public Task<Supplier?> GetSupplierAsync(TrustedProcurementMutationContext c, SupplierId id, CancellationToken ct) => Task.FromResult<Supplier?>(supplier); public Task<ProcurementOutcome> SaveSupplierAsync(TrustedProcurementMutationContext c, Supplier supplier, long? revision, CancellationToken ct) => Task.FromResult(ProcurementOutcome.Applied); public Task<PurchaseOrder?> GetPurchaseOrderAsync(TrustedProcurementMutationContext c, PurchaseOrderId id, CancellationToken ct) => Task.FromResult(Order); public Task<ProcurementOutcome> SavePurchaseOrderAsync(TrustedProcurementMutationContext c, PurchaseOrder x, long? revision, CancellationToken ct) { Order = x; return Task.FromResult(ProcurementOutcome.Applied); } }
}
