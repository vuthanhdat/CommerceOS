...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--allow-cloud", action="store_true", help="operator consent for canonical cloud-eligible tasks")
    parser.add_argument("--host", default="127.0.0.1")
    parser.add_argument("--port", type=int, default=8765)
    parser.add_argument("--no-browser", action="store_true")
    sub = parser.add_subparsers(dest="command", required=True)
    for name in ("status", "validate", "plan", "dry-run", "run", "stop", "resume", "cleanup", "ui", "start"):
        sub.add_parser(name)
    return parser


def build_orchestrator(args) -> tuple[TaskOrchestrator, RunStateStore]:
...
def cleanup(orchestrator: TaskOrchestrator, state: RunStateStore) -> int:
...
def main() -> int:
    ...
    try:
        ...
        if args.command == "validate":
            ...
            print_json({"valid": True, "tasks": len(snapshot.tasks), "ready_frontier": [t.id for t in BacklogReader.ready_frontier(snapshot, active_resources=set())]})
            return 0
