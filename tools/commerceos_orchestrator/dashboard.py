            ...
            def do_POST(self) -> None:  # noqa: N802
                ...
                try:
                    if parsed.path == "/api/stop":
                        ...
                        self._json(
                            {"accepted": True, "draining": ids}, HTTPStatus.ACCEPTED
                        )
                        return
...
async function refresh(){try{const r=await fetch('/api/status',{cache:'no-store'}),d=await r.json();if(!r.ok)throw Error(JSON.stringify(d));stateEl.textContent=`State: ${d.orchestrator_state}`;progressEl.textContent=`${d.progress.completed}/${d.progress.total} (${d.progress.percent}%)`;readyEl.textContent=d.ready_frontier.length;buildersEl.textContent=d.active_builders;reviewersEl.textContent=d.active_reviewers;mergeEl.textContent=d.merge_queue_length;blockedEl.textContent=d.blocker_count;for(const id of ['ready','active','review','merge','blocked'])byId('col-'+id).innerHTML=d.tasks.filter(t=>lane(t)===id).map(taskHtml).join('')||'<div class="small">None</div>';dagEl.innerHTML=d.tasks.map(t=>`<span title="deps: ${esc(t.depends_on.join(', '))}" class="node ${lane(t)} ${t.lifecycle_state==='Completed'?'completed':''}">${esc(t.id)}</span>`).join('');eventsEl.textContent=d.events.map(e=>`${e.created_at} ${e.task_id||'-'} ${e.kind} ${e.detail}`).join('\n')}catch(e){stateEl.textContent='Dashboard error: '+e}}
async function detail(id){const r=await fetch('/api/tasks/'+encodeURIComponent(id));detailEl.textContent=JSON.stringify(await r.json(),null,2)}async function stopRun(){await fetch('/api/stop',{method:'POST'});await refresh()}async function resumeRun(){await fetch('/api/resume',{method:'POST'});await refresh()}refresh();setInterval(refresh,1500);
</script></body></html>'''
